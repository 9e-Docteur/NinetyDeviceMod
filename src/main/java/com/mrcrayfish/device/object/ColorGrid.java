...
public class ColorGrid extends Component
{
	...
	public void render(PoseStack poseStack, Laptop laptop, Minecraft mc, int x, int y, int mouseX, int mouseY, boolean windowActive, float partialTicks)
	{
		...
		fill(poseStack, xPosition, yPosition, endX, endY, Color.DARK_GRAY.getRGB());
		for(int i = 0; i < colors.length; i++)
		{
			int startX = xPosition + (i % 5) * 10 + 1;
			int startY = yPosition + (i / 5) * 10 + 1;
			fill(poseStack, startX, startY, startX + 10, startY + 10, colors[i].getRGB());
		}
		...
		if(GuiHelper.isMouseInside(mouseX, mouseY, xPosition + 1, yPosition + 1, endX - 2, endY - 2))
		{
			...
			fill(poseStack, xPosition + (boxX * 10) + 1, yPosition + (boxY * 10) + 1, xPosition + (boxX * 10) + 11, yPosition + (boxY * 10) + 11, hoverColor);
		}
	}
